...
## <mark style="color:yellow;">Revoluzion Airdrop / Multi-Sender</mark>
...
### <mark style="color:yellow;">What is The Difference Between Both?</mark>
...
### <mark style="color:yellow;">Revoluzion Airdrop / Multi-Sender Source Code</mark>

<details>

<summary><mark style="color:yellow;">Source Code</mark></summary>
...
contract Airdrop is Ownable {
    ...
    /**
     * @dev Function to airdrop ERC20-based token with similar specific values to respective addresses.
     */
    function dropSimilarSpecificTokens(
        ...
        for (
            ...
            // Make sure that the token can transfer the amount to the address
            require(ERC20(_token).transfer(_recipients[i], _amount));
    ...
    /**
     * @dev Function to airdrop native token with different specific values to respective addresses.
     */
    function dropDifferentSpecificNative(
        address[] memory _recipients,
        uint256[] memory _amount
    ) public payable onlyOwner returns (bool) {
        
        // Initialize total native token to be dropped
        uint256 total = 0;

        // Get the total native token to be dropped
        for(
            uint256 j = 0;
            j < _amount.length;
            j++
            ...
            // Add the respective native token amount to the total amount
            total = total.add(_amount[j]);

        }

        // Make sure that total native token to be dropped is lesser or equal the total amount to be used
        require(
            total <= msg.value,
            "Amount sent to be dropped is too low than total airdrop."
        );

        // Make sure that the length of receipient array is equal to the length of native token airdrop array
        require(
            _recipients.length == _amount.length,
            "The length of recipient array is not equal to the length of native token airdrop array."
        );

        // Transfer native token to the addresses for the airdrop
        for (
            uint256 i = 0;
            i < _recipients.length;
            i++
            ...
            require(
                ...
                "Cannot transfer the airdrop native token to zero address."
            );

            // Transfer native token to the respective address for the airdrop
            payable(_recipients[i]).transfer(_amount[i]);

            // Log the individual transfer of the native token being airdrop
            emit NativeTransfer(_recipients[i], _amount[i]);
    ...
    /**
     * @dev Function to airdrop native token with similar specific values to respective addresses.
     */
    function dropSimilarSpecificNative(
        address[] memory _recipients,
        uint256 _amount
    ) public payable onlyOwner returns (bool) {
        
        // Initialize total native token to be dropped
        uint256 total = _amount.mul(_recipients.length);
        ...
        for (
            ...
            // Log the individual transfer of the native token being airdrop
            emit NativeTransfer(_recipients[i], _amount);
    ...
    /**
     * @dev Function to allow owner to withdraw ERC20-based token in smart contract.
     */
